/**
 * =========================================================================================
 * Project     : ESP32 Industrial SCADA RTU Gateway (DNP3 / IEC 60870-5-104)
 * Platform    : ESP32 (Dual-Core Xtensa LX6 / FreeRTOS)
 * Framework   : Arduino IDE 2.0+
 * Author      : Muhammad Fikri
 * License     : MIT
 * Description : Smart substation telemetry Remote Terminal Unit (RTU) supporting binary inputs, analog inputs, and select-before-operate control relays with sequence of events (SOE) timestamping.
 * =========================================================================================
 */
...
void TaskTelemetryBroadcaster(void *pvParameters) {
    ...
    for (;;) {
        if (xSemaphoreTake(g_telemetryMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            doc["system"] = "esp32-scada-dnp3-rtu-gateway";
...
void setup() {
    ...
    Serial.println(F("================================================================="));
    Serial.println(F(" ESP32 Industrial SCADA RTU Gateway (DNP3 / IEC 60870-5-104)"));
